...
const InvoiceCard = ({type="Invoice"}:any) => {
    ...
    <div className='sm:h-32 bg-[#F4F4F5] w-full text-black p-2 sm:p-3 rounded-2xl inter'>
        <div className='flex sm:flex-row flex-col sm:items-center justify-between'>
            <div className='space-y-2 text-sm inter'>
                <h2>{type} 3c779b</h2>
                 <h2>{type === "Invoice"? "Next Payment Due 10 October 2025 (1 day left)" : "Pending"}</h2>
            </div>
        ...
        <div className='flex items-center justify-between mt-3 '>
            <div className='flex items-center gap-4'>
                 <button onClick={()=>router.push(`/add${type}?id=1`)} className='cursor-pointer rounded-4xl  p-1 border-[#01B0E9] border-1  flex items-center gap-2 px-3'><BsSend className='w-4 h-4'/><span> View</span></button>
