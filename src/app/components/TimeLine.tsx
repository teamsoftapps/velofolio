...
const initialTasks = [
  { id: 1, title: "Lead Created", date: "25 Nov 2025", checked: true, tag: [""], type: "step" },
  {
    id: 2,
    title: "Job Accepted",
    date: "26 Nov 2025",
...
export default function TaskList() {
  ...
  return (
    <div className="w-full h-full py-3 flex flex-col gap-7">
      {tasks.map((task, index) => {
        const borderColor = task.checked ? "border-green-600" : "border-blue-400";
        ...
        return (
          ...
          >
            ...
            {index !== tasks.length - 1 && (
              <span
                className={`absolute w-2 rounded-l-lg top-1/2 -left-3 border-l-2 ${borderColor}`}
                style={{ height: lineHeights[index] ? `${lineHeights[index]}px` : "100%" }}
              ></span>
            )}

            {/* Card */}
            <div className={`relative w-[92%] bg-white border-1 ${borderColor} rounded-lg ${task.type === "step" ? "p-2" : ""}`}>
              {/* Circle */}
              <span
                className={`circle w-4 h-4 border-2 ${borderColor} bg-white rounded-full absolute -left-3 top-1/2 -translate-y-1/2 z-10`}
              ></span>

              {/* Row (title + checkbox + right side) */}
              <div className="flex justify-between items-center" ref={el => { rowRefs.current[index] = el }}>
                <div className="p-2">
  ...
  <span
    className={`w-4 h-4 rounded border-2 flex items-center justify-center
      ${task.checked ? "bg-green-500 border-green-500" : "bg-white border-gray-400"}
      ${isDisabled ? "bg-gray-200 border-gray-300" : ""}
    `}
  >
    {task.checked && <CheckIcon className="w-3 h-3 text-white" />}
  </span>
  <span className={isDisabled ? "text-gray-400" : ""}>{task.title}</span>
</label>
                ...
                {task.type === "step" ? (
                  ...
                  <div className="flex items-stretch gap-2">
                    ...
                    <div
                      className={`w-20 border-l-2 ${borderColor} flex items-center justify-center`}
