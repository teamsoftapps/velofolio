...
// // const AddInvoiceModal: React.FC<AddInvoiceModalProps> = ({ isOpen, onClose, onSubmit,setInvoices }) => {
...
// //     // Fix: Properly spread the previous state inside the updater function
// //     setInvoices((prev: any) => [...prev, formData]);
// //     onClose();
// //   }
// // };
...
// //       <div
// //         className={`
// //           relative min-w-74 w-full h-48 rounded-lg border-2 p-2 border-dashed transition-all duration-200
// //           ${isDragging
...
// //       </div>
// //     </div>
// //               <ImageReplacementTab imagePlacement={imagePlacement} setImagePlacement={setImagePlacement} />
// //             </div>
// //           </div>
...
// //           onChange={()=>setFormData(prev => ({ ...prev, description: prev.description }))}
...
// //                 <h3 className='font-bold  text-xl my-1'>${calculateTotalAmount()}</h3>
// //             </div>
...
// //           <button className='w-full sm:w-40 bg-[#01B0E9] text-white py-3 rounded-full mb-2 sm:mb-0 cursor-pointer'>Save Invoice</button> 
// //           <button 
// //             onClick={() => onClose()}
...
//                 </div>
//               </div>
//               <ImageReplacementTab imagePlacement={formData.imagePlacement} setImagePlacement={(value: 'above' | 'below') => handleInputChange('imagePlacement', value)} />
//             </div>
//           </div>
...
interface AddItemData {
  ...
  quantity: string;
  tax: string;
  totalAmount: string; // Added totalAmount to the form data
}
...
const AddInvoiceModal: React.FC<AddInvoiceModalProps> = ({ isOpen, onClose, onSubmit, setInvoices }) => {
  ...
  const newFormData = { ...formData, id };
    ...
    if (formData.name.trim()) {
      setInvoices((prev: any) => [...prev, newFormData]);
      onClose();
    }
  };
